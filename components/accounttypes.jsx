...
const onChangeAcctType = (index, acctLocale) => {
  ...
  console.log(
    ...
    new Intl.NumberFormat(acctLocale.locale, acctLocale.format).format(
      accountTypeList[index].price
    ),
    acctLocale.perMonth
  );
};
...
//... Main class
const AccountTypesComp = ({ acctLocale }) => (
  <>
    <div>
      <FieldArray
        name="accountTypeId"
            ...
            {accountTypeList.map((accountType, index) => (
              <section className="radiobutton" key={accountType.id}>
                <input
                  ...
                  onChange={e => {
                    e.target.checked
                      ? onChangeAcctType(index, acctLocale)
                      : null;
                  }}
                />
                &nbsp;
...
export default AccountTypesComp;
